...
1. 【身分與渠道檢驗】：發送者是否冒充金融機構、公務機關、知名電商或親友？發送渠道（如非官方短網址、奇怪國碼電話）是否存在偽冒跡象？注意：台灣各大官方電信與正派機構之合法專屬網域（包含台灣大哥大 `twm.tw`/`taiwanmobile.com`、中華電信 `cht.tw`、遠傳電信 `fet.tw`/`fetnet.net`、政府 `*.gov.tw`、學校 `*.edu.tw`、各大銀行官方網站）屬於合法官方渠道，非釣魚網址。
2. 【利益誘餌與損失威脅】：訊息中是否提供異常高報酬（保證獲利、先付稅金領大獎）、或者利用損失恐懼（即將扣款、帳戶凍結、違法通緝）施壓？正派電信業者之會員活動與促銷（如台灣大哥大 twm.tw 門號抽獎參加資格、老客戶續約回饋、中華電信 cht.tw PC-cillin/防駭加值推廣）屬於正常商業推廣與會員回饋，非詐騙。
...
5. 【防誤判機制 (Guardrail)】：
   ...
   - 或為正派電信業者（台灣大哥大 twm.tw、中華電信 cht.tw、遠傳 fet.tw）發送之會員活動、門號抽獎資格、加值方案行銷，且無要求先支付稅金/手續費、無索取銀行帳密或 OTP 驗證碼者。
   - 必須客觀判定為【安全 (正常訊息)】，風險指數 0 ~ 20 分，切勿將電信官方合法短網址（twm.tw / cht.tw / fet.tw）或正派行銷活動誤判為詐騙！"""
...
- 釣魚簡訊/惡意短網址：偽冒「監理站罰款」、「水電費逾期」、「健保卡停卡」，附帶非官方境外網域（如 .vip, .top, .xyz, .cc 等非 .gov.tw 網址）騙取信用卡號或 OTP。注意：官方網域如 twm.tw, cht.tw, fet.tw, gov.tw, edu.tw 絕非此類。
...
- 正常訊息對照組：電信官方會員活動（如台灣大哥大 twm.tw 抽獎活動資格通知、中華電信 cht.tw 加值推廣）、銀行刷卡通知、超商取件、學校選課通知、醫院預約看診，均為安全正常訊息。"""
